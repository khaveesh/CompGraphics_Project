...
export default class Application {
  constructor() {
    ...
    this.debug_camera = new THREE.PerspectiveCamera(
      75,
    ...
    document.addEventListener(
      "keypress",
      (event) => this.keypress_handler(event),
    );
    document.addEventListener(
      "keydown",
      (event) => this.keydown_handler(event),
    );
  ...
  animate() {
    if (this.fixedCam) {
      this.renderer.render(this.scene, this.debug_camera);
    } else {
  ...
  initialize_scene() {
    ...
    this.objects.push(
      new NPC(this.scene, "cube.obj", [20, 1, -20], [0, 0, 0], 1, []),
    );

    this.objects.push(
      new NPC(this.scene, "house1.obj", [70, 5, -20], [0, Math.PI, 0], 5, []),
    );
